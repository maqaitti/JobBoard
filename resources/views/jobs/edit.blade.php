            ...
            <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
              <input id="salary"
               type="text"
                ...
                class="block min-w-0 grow bg-white py-1.5 px-4 pl-1 text-base text-gray-900 placeholder:text-gray-400 focus:outline-none sm:text-sm/6" 
              value="{{ $job->title }}"
              required>
            </div>
            @error('salary')
...
<form method="POST" action="/jobs/{{ $job->id }}" id="delete-form" class="hedden">
  @csrf
  @method('DELETE')
</form>
</x-layout>